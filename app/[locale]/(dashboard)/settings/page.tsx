...
export default function SettingsPage() {
  ...
  const handleUpdate = async (data: UserFormData) => {
    ...
    try {
      ...
      toast.error(error.message || t('profile.errorUpdate'));
    } finally {
    ...
    <div className="min-h-screen flex flex-col items-center pt-6 pb-20 px-4">
      ...
      <div className="absolute top-4 left-4">
        <Button variant="ghost" size="icon" asChild className="rounded-full text-gray-400 hover:text-gray-900 transition-colors">
            <Link href="/">
                <ChevronLeft className={cn("h-6 w-6", locale === 'ar' && "rotate-180")} />
            </Link>
        </Button>
      </div>
      ...
      <div className="w-full max-w-sm space-y-6 flex flex-col items-center">
        ...
        <div className="text-center space-y-3">
            <div className="space-y-1">
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{roleLabel}</p>
            ...
            <div className="flex items-center justify-center gap-2 text-gray-600 bg-gray-50 px-5 py-2.5 rounded-full border border-gray-100 shadow-sm">
                ...
                <span className="text-sm font-bold">{user?.phone || t('profile.phoneNotSet')}</span>
            </div>
        </div>
            ...
            {user?.role === "ADMIN" && (
                <div className="pt-4 border-t border-gray-100 space-y-4 w-full">
                    <div className="text-center space-y-1">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{t('system.title')}</p>
                    ...
                    <div className="bg-white p-4 rounded-3xl border border-gray-100 shadow-sm space-y-4">
                        ...
                        <div className="space-y-1.5">
                            <Label htmlFor="batchSize" className="text-[10px] font-black text-gray-400 uppercase tracking-wider ml-1">{t('system.batchLabel')}</Label>
                            <div className="flex gap-2">
                                ...
                                <Button 
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            toast.success(t('system.batchSuccess'));
                                            await fetchData();
                                        } finally {
                                ...
                                >
                                    {isSavingBatch ? <RefreshCw className="h-3 w-3 animate-spin" /> : t('system.applyBtn')}
                                </Button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-1.5 pt-3 border-t border-gray-50">
                            <Label htmlFor="maxRecall" className="text-[10px] font-black text-gray-400 uppercase tracking-wider ml-1">{t('system.recallLabel')}</Label>
                            <div className="flex gap-2">
                                <Input 
                                    id="maxRecall"
                                    type="number" 
                                ...
                                <Button 
                                    onClick={async () => {
                                        ...
                                        try {
                                            await updateSystemSettings({ assignmentBatchSize: batchSize, maxRecallAttempts });
                                            toast.success(t('system.recallSuccess'));
                                            await fetchData();
                                        } finally {
                                    ...
                                    className="h-10 flex-1 rounded-xl bg-[#1F30AD] hover:bg-[#172585] text-white text-[10px] font-black uppercase tracking-widest transition-all"
                                >
                                    {isSavingRecall ? <RefreshCw className="h-3 w-3 animate-spin" /> : t('system.applyBtn')}
                                </Button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="text-center space-y-1 pt-4">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">{t('schedule.title')}</p>
                        <h2 className="text-sm font-black text-gray-900 tracking-tight uppercase">{t('schedule.subtitle')}</h2>
                    </div>
                    ...
                    <div className="bg-white p-4 rounded-3xl border border-gray-100 shadow-sm space-y-4 w-full">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-wider ml-1">{t('schedule.startWork')}</Label>
                                <Input 
                                    type="time" 
                                    value={workStart}
                            ...
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-wider ml-1">{t('schedule.endWork')}</Label>
                                <Input 
                                    type="time" 
                                    value={workEnd}
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-3 border-t border-gray-50">
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-wider ml-1">{t('schedule.startBreak')}</Label>
                                <Input 
                                    type="time" 
                                    value={breakStart}
                            ...
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-black text-gray-400 uppercase tracking-wider ml-1">{t('schedule.endBreak')}</Label>
                                <Input 
                                    type="time" 
                                    value={breakEnd}
                        ...
                        <div className="pt-3 border-t border-gray-50 space-y-2">
                             <Label className="text-[10px] font-black text-gray-400 uppercase tracking-wider ml-1">{t('schedule.workingDays')}</Label>
                        ...
                        <Button 
                            onClick={async () => {
                                ...
                                try {
                                    ...
                                    toast.success(t('schedule.successUpdate'));
                                    await fetchData();
                                } catch (error) {
                        ...
                        >
                            {isSavingSchedule ? <RefreshCw className="h-4 w-4 animate-spin" /> : t('schedule.saveBtn')}
                        </Button>
                    </div>
                </div>
            )}
      ...
      <UserFormModal
        ...
        description={t('profile.modalDesc')}
      />
