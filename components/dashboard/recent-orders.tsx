        ...
        <div className="space-y-4">
          {!orders || orders.length === 0 ? (
            ...
            orders.map((order) => (
              <div
                key={order.id}
                className="flex items-center justify-between p-4 rounded-xl border border-slate-50 bg-white hover:bg-blue-50/30 hover:border-blue-100 transition-all duration-300 group shadow-xs"
              >
                <div className="flex items-center gap-4">
                  <div className="h-10 w-10 rounded-xl bg-blue-100 flex items-center justify-center text-[#172585] font-black text-[10px] shadow-xs group-hover:scale-110 transition-transform">
                    #{order.orderNumber}
                  </div>
                  <div>
